...
> 说明：当前活动入口已经统一到上一层的 `send/damiao.py`。本文件只保留历史流程说明，相关旧脚本已归档在当前目录。
...
```shell
cd ~/catkin_ws/u2canfd/send/old
python3 dev_sn.py
```
如果你只想在命令行里直接拿到SN，方便复制或继续传给别的命令，也可以这样运行：
```shell
cd ~/catkin_ws/u2canfd/send/old
python3 dev_sn.py --sn-only
```
如果当前没有识别到设备，脚本会明确输出：
```shell
未找到 U2CANFD 设备 (VID: 0x34B7, PID: 0x6877)。
```
...
```
***注意：上面的"14AA044B241402B10DDBDAFE448040BB"是我设备的SN号，你要替换为你的SN号，通过运行当前目录里的 `dev_sn.py` 文件就可以找到你设备的SN号，前面也提到过***
